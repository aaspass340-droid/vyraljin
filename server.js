...
app.get('/health', (req, res) => res.json({ status: 'ok', ver: 'v8.0-quality', ffmpeg: FFMPEG_BIN, bunny: !!BUNNY_KEY, gemini: !!GEMINI_KEY }));
...
app.post('/api/render', (req,res,next)=>{ _lastRenderErr='STEP 0: /api/render request aayi! '+new Date().toISOString(); next(); }, upload.fields([{name:'video',maxCount:1},{name:'overlay',maxCount:1}]), (req, res) => {
  ...
  function doRender() {
    ...
    const args = of
      ? ['-y','-filter_complex_threads','1',...trimArgs,'-i',of.path,'-filter_complex',fcOv,'-map','[outv]','-c:v','libx264','-preset','veryfast','-threads','1','-crf','18','-maxrate','6M','-bufsize','12M','-pix_fmt','yuv420p','-an','-movflags','+faststart','-max_muxing_queue_size','1024',out]
      : ['-y','-filter_threads','1',...trimArgs,'-vf',scaleF,'-c:v','libx264','-preset','veryfast','-threads','1','-crf','18','-maxrate','6M','-bufsize','12M','-pix_fmt','yuv420p','-an','-movflags','+faststart','-max_muxing_queue_size','1024',out];
